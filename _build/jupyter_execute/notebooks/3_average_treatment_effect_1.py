...
# 
# \begin{equation}\label{eq:ate} \tag{3.1}
...
# 
# \begin{equation}\label{eq:overlap} \tag{3.4}
...
# 
# \begin{equation}\label{eq:ate-dm} \tag{3.5}
#   \widehat{\tau}^{DIFF} =
...
# \begin{equation} \label{eq:agg} \tag{3.6} 
#   \overbrace{
...
# Next, we'll manipulate the expression (3.6). This next derivation can be a bit overwhelming, but please keep in mind that we're simply doing algebraic manipulations, as well as establishing some common and useful notation. First, note that we can rewrite the average for school A in (3.6) as
# 
# \begin{equation}\label{eq:avg1} \tag{3.7}
...
# \end{equation}
# 
# where $W_i$ is the treatment indicator. Plugging \eqref{eq:avg1} back into (3.6),
# 
# \begin{equation}
#   \left ({\frac{n_{A}}{n}}\right) 
#     \frac{1}{n_A} \sum_{i \in A} \frac{W_i}{(n_{A,1} / n_A)} Y_i
...
# \end{equation}
# 
# Note that the $n_A$ and $n_B$ will cancel out. Finally, if we denote the sample proportion of treated students in school $A$ by $\hat{e}(A_i) \approx n_{A,1}/n_A$ and similar for school $B$, (3.6) can be written compactly as
# 
# \begin{equation} \label{ipw-treated} \tag{3.8}
#   \frac{1}{n} \sum_{i=1}^{n}  \frac{W_i}{\hat{e}(X_i)} Y_i,
# \end{equation}
# 
# where $X_i \in \{A, B\}$ denotes the school membership. Quantity $\hat{e}(X_i)$ is an estimate of the probability of treatment given the control variable $e(X_i) = \mathbb{P}[W_i=1 | X_i ]$, also called the **propensity score**. Because \eqref{ipw-treated} is a weighted average with weights $W_i/\hat{e}(X_i)$, when written in this form we say that it is an **inverse propensity-weighted estimator** (IPW).
# 
# The IPW estimator is also defined for the case of continuous covariates. In that case, we must estimate the assignment probability given some model, for example using logistic regression, forests, etc. With respect to modeling, the behavior of the IPW estimator is similar to the direct estimator: if $\hat{e}(X_i)$ is an unbiased estimate of $e(X_i)$, then \eqref{ipw-treated} is an unbiased estimate of $E[Y(1)]$; and we can show that if $\hat{e}(X_i)$ is a consistent estimator of $e(X_i)$, then \eqref{ipw-treated} is consistent for $E[Y(1)]$. 
# 
# Another important point is that when the estimated treatment propensity $\hat{e}(X_i)$ is small, the summands in \eqref{ipw-treated} can be very large. In particular, if $\hat{e}(x)$ is exactly zero, then this estimator is undefined. That is why, in addition to requiring conditional unconfoundedness (3.3), it also requires the overlap condition (3.4). In any case, when overlap is small (i.e., $\hat{e}(x)$ is very close to zero for many $x$), IPW becomes an unattractive estimator due to its high variance. We'll see in the next section an improved estimator that builds upon IPW but is strictly superior and should be used instead.
...
# 
# The next **augmented inverse propensity-weighted estimator** (AIPW) of the treatment effect is available under unconfoundedness (3.3) and overlap (3.4):
# 
# \begin{equation} \label{eq:aipw} \tag{3.10}
#   \begin{aligned}
#   \hat{\tau}^{AIPW} &:=
#     \frac{1}{n} \sum_{i=1}^{n}
...
# \end{equation}
# 
# Let’s parse \eqref{eq:aipw}. Ignoring the superscripts for now, the first two terms correspond to an estimate of the treatment effect obtained via direct estimation. The next two terms resemble the IPW estimator, except that we replaced the outcome $Y_i$ with the residuals $Y_i - \hat{\mu}(X_i, W_i)$. At a high level, the last two terms estimate and subtract an estimate of the bias of the first.
# 
